...
#
# By default, downloads to ~/.boundry/weights/
# Override with BOUNDRY_WEIGHTS_DIR environment variable

set -e

# Determine weights directory
if [ -n "$BOUNDRY_WEIGHTS_DIR" ]; then
    WEIGHTS_DIR="$BOUNDRY_WEIGHTS_DIR"
else
    WEIGHTS_DIR="$HOME/.boundry/weights"
fi
