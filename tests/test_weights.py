"""Tests for boundry.weights module.
...
class TestGetWeightsDir:
    ...
    def test_default_location(self):
        """Test default weights directory is ~/.boundry/weights."""
        with patch.dict(os.environ, {}, clear=True):
            # Remove BOUNDRY_WEIGHTS_DIR if set
            os.environ.pop("BOUNDRY_WEIGHTS_DIR", None)
            result = get_weights_dir()
            assert result == Path.home() / ".boundry" / "weights"

    def test_env_override(self, tmp_path):
        """Test BOUNDRY_WEIGHTS_DIR environment variable override."""
        custom_dir = tmp_path / "custom_weights"
        with patch.dict(
            os.environ, {"BOUNDRY_WEIGHTS_DIR": str(custom_dir)}
        ):
            result = get_weights_dir()
    ...
    def test_env_override_empty_string(self):
        ...
        with patch.dict(os.environ, {"BOUNDRY_WEIGHTS_DIR": ""}):
            result = get_weights_dir()
...
class TestGetPackageWeightsDir:
    ...
    def test_returns_package_path(self):
        ...
        # Should be in src/boundry/LigandMPNN/model_params
...
class TestFindWeightsDir:
    ...
    def test_finds_user_dir_when_weights_exist(self, tmp_path):
        ...
        with patch.dict(os.environ, {"BOUNDRY_WEIGHTS_DIR": str(user_dir)}):
    ...
    def test_finds_package_dir_when_user_empty(self, tmp_path):
        ...
        with patch.dict(os.environ, {"BOUNDRY_WEIGHTS_DIR": str(user_dir)}):
            with patch(
                "boundry.weights._get_package_weights_dir",
...
class TestWeightsExist:
    ...
    def test_returns_true_when_all_files_exist(self, tmp_path):
        ...
        with patch.dict(
            os.environ, {"BOUNDRY_WEIGHTS_DIR": str(weights_dir)}
        ):
    ...
    def test_raises_permission_error_on_mkdir_failure(self, tmp_path):
        ...
        with patch.dict(
            ...
            with patch(
                ...
                with pytest.raises(
                    PermissionError, match="BOUNDRY_WEIGHTS_DIR"
                ):
                    download_weights(verbose=False)
...
class TestEnsureWeights:
    ...
    def test_calls_download_weights(self, tmp_path):
        ...
        with patch.dict(
            ...
            with patch("boundry.weights.download_weights") as mock_download:
