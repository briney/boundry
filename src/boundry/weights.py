...
def get_weights_dir() -> Path:
    ...
    Uses ~/.boundry/weights/ as the primary location, following
    conventions used by other biological ML tools (boltz, chai, etc.).

    The directory can be overridden via the BOUNDRY_WEIGHTS_DIR
    environment variable.

    Returns:
        Path to the weights directory
    """
    if env_dir := os.environ.get("BOUNDRY_WEIGHTS_DIR"):
        return Path(env_dir)
    return Path.home() / ".boundry" / "weights"
...
def find_weights_dir() -> Path:
    ...
    1. User weights directory (~/.boundry/weights/)
    2. Environment override (BOUNDRY_WEIGHTS_DIR)
...
def download_weights(verbose: bool = True) -> None:
    ...
    Downloads to ~/.boundry/weights/ by default.
    ...
    try:
        ...
        raise PermissionError(
            ...
            f"Set BOUNDRY_WEIGHTS_DIR to a writable location. Error: {e}"
        ) from e
    except OSError as e:
        raise RuntimeError(
